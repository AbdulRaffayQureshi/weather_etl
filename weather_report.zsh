...
# 1. Extract (Added 'm' to force Metric units for US-based cloud servers)
curl -s -A "Mozilla/5.0" "wttr.in/Islamabad?Tm" > $weather_report
...
if [[ -n "$obs_tmp" && -n "$fc_temp" ]]; then
    ...
    echo "--- Debugging: Raw file contents below ---"
    cat $weather_report
    exit 1
fi
